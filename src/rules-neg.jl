...
theories = (;
    ...
    distributivity_fp_safe=(;
        groups=(:arithmetic, :simplify, :fp_safe, :sound),
        rules=[
            @rule "distribute-lft-neg-in" a b neg((a * b)) --> (neg(a) * b)
            @rule "distribute-rgt-neg-in" a b neg((a * b)) --> (a * neg(b))
            @rule "distribute-rgt-neg-in" a b neg((a * b)) --> (a * neg(b))
            @rule "distribute-lft-neg-out" a b (neg(a) * b) --> neg((a * b))
            @rule "distribute-rgt-neg-out" a b (a * neg(b)) --> neg((a * b))
            @rule "distribute-neg-in" a b neg((a + b)) --> (neg(a) + neg(b))
            @rule "distribute-neg-out" a b (neg(a) + neg(b)) --> neg((a + b))
            @rule "distribute-frac-neg" a b (neg(a) / b) --> neg((a / b))
            @rule "distribute-frac-neg2" a b (a / neg(b)) --> neg((a / b))
            @rule "distribute-neg-frac" a b neg((a / b)) --> (neg(a) / b)
            @rule "distribute-neg-frac2" a b neg((a / b)) --> (a / neg(b))
        ],
    ),
    cancel_sign_fp_safe=(;
        groups=(:arithmetic, :simplify, :fp_safe, :sound),
        rules=[
            @rule "cancel-sign-sub" a c b (a - (neg(b) * c)) --> (a + (b * c))
            @rule "cancel-sign-sub-inv" a c b (a - (b * c)) --> (a + (neg(b) * c))
        ],
    ),
    ...
    id_reduce_fp_safe=(;
        ...
        rules=[
            ...
            @rule "--rgt-identity" a (a - 0) --> a
            @rule "sub0-neg" a (0 - a) --> neg(a)
            @rule "remove-double-neg" a neg(neg(a)) --> a
            @rule "*-lft-identity" a (1 * a) --> a
            @rule "*-rgt-identity" a (a * 1) --> a
            @rule "/-rgt-identity" a (a / 1) --> a
            @rule "mul-1-neg" a (-1 * a) --> neg(a)
        ],
    ),
    nan_transform_fp_safe=(;
        groups=(:arithmetic, :simplify, :fp_safe, :sound),
        rules=[
            @rule "sub-neg" a b (a - b) --> (a + neg(b))
            @rule "unsub-neg" a b (a + neg(b)) --> (a - b)
            @rule "neg-sub0" b neg(b) --> (0 - b)
            @rule "neg-mul-1" a neg(a) --> (-1 * a)
        ],
    ),
    id_transform_safe=(;
    ...
    #    @rule "asin-sin" x asin(sin(x)) --> (fabs(((x+(pi/2))remainder(2*pi)))-(pi/2))
    #    @rule "acos-cos" x acos(cos(x)) --> fabs((xremainder(2*pi)))
    #]),
