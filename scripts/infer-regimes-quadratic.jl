...
function infer_regimes(
    original::Expression{T}, candidates, splits, points; last_point=plus_infsplit(splits[1])
) where {T}
    function _best_candidate(low, high)
        ops = original.metadata.operators
        vars = original.metadata.variable_names
        (_, c) = best_candidate(original.tree, [c.tree for c in candidates], ops, points, low, high)
...
splits = [
    (T(-1e3), 2), (T(-1e2), 2), (T(-1e1), 2), (T(-1e0), 2), (T(1e1), 2), (T(1e2), 2), (T(1e3), 2)
]
regs = infer_regimes(dexpr, candidates, splits, points)
display(regs)
println("")
